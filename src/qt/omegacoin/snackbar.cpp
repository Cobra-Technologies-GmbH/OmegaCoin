...
#include "qt/omegacoin/forms/ui_snackbar.h"
#include "qt/omegacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(OMEGACOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &OMEGACOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
