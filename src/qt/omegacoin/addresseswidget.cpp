...
#include "qt/omegacoin/forms/ui_addresseswidget.h"
#include "qt/omegacoin/addresslabelrow.h"
#include "qt/omegacoin/addnewaddressdialog.h"
#include "qt/omegacoin/tooltipmenu.h"

#include "qt/omegacoin/addnewcontactdialog.h"
...
AddressesWidget::AddressesWidget(OMEGACOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto omegaAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(omegaAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(omegaAdd);
        if (walletModel->updateAddressBookLabels(omegaAdd, label.toUtf8().constData(),
