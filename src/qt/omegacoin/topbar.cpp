...
#include "qt/omegacoin/forms/ui_topbar.h"
#include "qt/omegacoin/lockunlock.h"
#include "qt/omegacoin/qtutils.h"
#include "qt/omegacoin/receivedialog.h"
#include "qt/omegacoin/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/omegacoin/balancebubble.h"
...
TopBar::TopBar(OMEGACOINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopOmega, ui->labelAmountTopShieldedOmega}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOmega}, "amount-topbar");
    setCssProperty({ui->labelPendingOmega, ui->labelImmatureOmega}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("OMEGACOIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // OMEGA Total
    QString totalOmega = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // OMEGA
    // Top
    ui->labelAmountTopOmega->setText(totalTransparent);
    ui->labelAmountTopShieldedOmega->setText(totalShielded);
    // Expanded
    ui->labelAmountOmega->setText(totalOmega);
    ui->labelPendingOmega->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureOmega->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
