...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("OMEGACOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("OMEGACOINMiner : ProcessNewBlock, block not accepted");
    }
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("omegacoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OMEGACOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("OMEGACOINMiner exception");
    } catch (...) {
        LogPrintf("OMEGACOINMiner exception");
    }

    LogPrintf("OMEGACOINMiner exiting\n");
}
