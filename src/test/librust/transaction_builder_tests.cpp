...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-OMEGA in, 0.4 shielded-OMEGA out, 0.1 t-OMEGA fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling)
{
    ...
    // --- 0.4 shielded-OMEGA in, 0.299 shielded-OMEGA out, 0.1 shielded-OMEGA fee, 0.001 shielded-OMEGA change (added to fee)
    ...
    // --- Now try with 1 shielded-OMEGA in, 0.5 shielded-OMEGA out, 0.1 shielded-OMEGA fee, 0.4 shielded-OMEGA change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 OMEGA note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-OMEGA out, 0.1 t-OMEGA fee
    ...
    // 0.5 t-OMEGA out, 0.1 t-OMEGA fee
    ...
    // 0.5 t-OMEGA out, 0.1 t-OMEGA fee, 0.59999 shielded-OMEGA in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
