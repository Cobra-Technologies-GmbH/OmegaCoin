...
bool CLegacyZOmegaStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZOmegaStake from non zc-spend", __func__);
    ...
    *this = CLegacyZOmegaStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zomega stake origin", __func__);

    // All good
    return true;
}

CLegacyZOmegaStake::CLegacyZOmegaStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZOmegaStake::GetIndexFrom() const
{
...
CAmount CLegacyZOmegaStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZOmegaStake::GetUniqueness() const
{
...
bool CLegacyZOmegaStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zOmega stake block: height %d outside range", __func__, nHeight);
