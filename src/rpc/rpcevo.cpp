...
static const std::map<ProRegParam, std::string> mapParamHelp = {
        {collateralAddress,
            "%d. \"collateralAddress\"     (string, required) The OMEGACOIN address to send the collateral to.\n"
        },
        {collateralHash,
        ...
        {ownerAddress,
            "%d. \"ownerAddress\"          (string, required) The OMEGACOIN address to use for payee updates and proposal voting.\n"
        ...
        {payoutAddress_register,
            "%d. \"payoutAddress\"          (string, required) The OMEGACOIN address to use for masternode reward payments.\n"
        },
        {payoutAddress_update,
            "%d. \"payoutAddress\"          (string, required) The OMEGACOIN address to use for masternode reward payments.\n"
...
// Allows to specify OMEGACOIN address or priv key (as strings). In case of OMEGACOIN address, the priv key is taken from the wallet
...
static CKeyID ParsePubKeyIDFromAddress(const std::string& strAddress)
{
    ...
    if (!keyID) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("invalid OMEGACOIN address %s", strAddress));
    }
    return *keyID;
}
...
UniValue protx_register_fund(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 6 || request.params.size() > 8) {
        throw std::runtime_error(
                ...
                "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 10000 OMEGA\n"
