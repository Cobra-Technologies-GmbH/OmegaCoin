...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/OMEGACOINParams"
    else
        PARAMS_DIR="$HOME/.omegacoin-params"
    fi
fi
...
Failed to fetch the OMEGACOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
OMEGACOIN - fetch-params.sh

This script will fetch the OMEGACOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common OMEGACOIN zkSNARK parameters. Note that it is
