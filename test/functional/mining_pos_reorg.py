...
class ReorgStakeTest(OmegacoinTestFramework):
    ...
    def check_money_supply(self, expected_omega):
        # verify that nodes have the expected OMEGA supply
        omega_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(omega_supply, [DecimalAmt(expected_omega)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that OMEGA supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check OMEGA supply...")
