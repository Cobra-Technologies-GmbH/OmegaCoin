...
TMPDIR_PREFIX = "omegacoin_func_test_"


class OmegacoinTestFramework():
    """Base class for a omegacoin test script.

    Individual omegacoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use omegacoin-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: omegacoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a omegacoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple omegacoind test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'omegacoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "omegacoind should have exited with an error"
                else:
                    assert_msg = "omegacoind should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as omegacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in omegacoin.conf
        ...
        def clone_cache_from_node_1(cachedir, from_num=4):
            ...
            for i in range(from_num, MAX_NODES):
                ...
                initialize_datadir(cachedir, i)  # Overwrite port/rpcport in omegacoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "omegacoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
    ...
    ### OMEGACOIN Specific TestFramework ###
    ###################################
...
### ----------------------
    ...
    """
    Create a ProReg tx, which references an 100 OMEGA UTXO as collateral.
...
'''
OmegacoinTestFramework extensions
'''
# !TODO: remove after obsoleting legacy system
class OmegacoinTier2TestFramework(OmegacoinTestFramework):
