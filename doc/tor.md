...
in `torrc` config file. For cookie authentication the user running omegacoind must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running omegacoind to the same group and setting permissions appropriately. On
Debian-based systems the user running omegacoind can be added to the debian-tor group,
...
- Do not add anything but OMEGACOIN Core ports to the hidden service created in section 2.
