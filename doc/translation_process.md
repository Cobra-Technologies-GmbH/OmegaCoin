...
Multiple language support is critical in assisting OMEGACOIN's global adoption, and growth. One of OMEGACOIN's greatest strengths is cross-border money transfers, any help making that easier is greatly appreciated.

See the [Transifex OMEGACOIN project](https://www.transifex.com/omegacoin-project/omegacoin-project-translations/) to assist in translations.
...
You can find the OMEGACOIN translation project at [https://www.transifex.com/omegacoin-project/omegacoin-project-translations/](https://www.transifex.com/omegacoin-project/omegacoin-project-translations/).
...
The Transifex OMEGACOIN project config file is included as part of the repo. It can be found at `.tx/config`, however you shouldn’t need to change anything.
...
3. Update `src/qt/omegacoin_locale.qrc` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(omegacoin_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'
```
4. Update `src/Makefile.qt_locale.include` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(omegacoin_\(.*\)\).ts/  qt\/locale\/\1.ts \\/'
```
...
To create a new language template, you will need to edit the languages manifest file `src/qt/omegacoin_locale.qrc` and add a new entry. Below is an example of the English language entry.
...
The OMEGACOIN Core translation maintainers include *Fuzzbawls*. You can find them, and others, in the [OMEGACOIN Discord](https://discord.omega.eco).
