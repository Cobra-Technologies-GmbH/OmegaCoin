...
OMEGACOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
OMEGACOIN Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_omegacoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_omegacoin --log_level=test_suite
$ valgrind -v --leak-check=full src/omegacoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_omegacoin.coverage/index.html`.
```
...
be sent upstream but bugfixes may also be prudent to PR against OMEGACOIN Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `omegacoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `omegacoin-cli` and the GUI debug console use this table to determine how to
