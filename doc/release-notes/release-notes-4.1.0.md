OMEGACOIN Core version *4.1.0* is now available from:  <https://github.com/Cobra-Technologies-GmbH/OmegaCoin/releases>
...
An extended description of this large work can be found in the PR [here](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1327).
...
Dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1392)).
...
Addresses in the dropdown of the "Send Transaction" and "Send Delegation" widgets are now automatically sorted by label with ascending order ([#1393](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1393)).

### Custom Fee

The custom fee selected when sending a transaction is now saved in the wallet database and persisted across multiple sends and wallet's restarts ([#1406](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1406)). The fee is now also validated against the maximum value (10000 times `minRelayTxFee`) and minimum value (`minTxFee`) ([#1576](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1576)).

### Include delegations in send

The send and cold-staking page present a checkbox to make the automatic input selection algorithm include delegated (P2CS) utxos if needed. The option is unchecked by default. ([#1391](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1391))

### Optional Hiding of Staking Charts

The staking charts can now be hidden at startup (with a flag `--hidecharts`) or at runtime with a checkbox in settings --> options --> display ([#1475](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1475)).
...
Restore the previous locking state ("locked" or "locked for staking only") when the action is completed. ([#1387](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1387))

### External Change Address Warning

A new warning/confirmation dialog is displayed if a custom change address is not part of the wallet. ([#1527](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1527))

### Cold Stake delegations marked in Coin Control

The Coin Control window now includes an icon next to the select checkbox when the UTXO is a Cold Stake delegation. This shares the space with the locked UTXO indicator icon, and locked UTXO's take priority in their icon display. ([#1470](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1470))

### Hide zOmega balance info as needed

When the wallet contains no zOmega, the zOmega balance details will be hidden, reducing visual clutter.
...
```
"Confirmed","Date","Type","Label","Address","Amount (OMEGA)","ID"
```
...
### zOmega Backup Removed

Automatic zOmega backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the omegacoin.conf file or as startup flags):
- `autozomegabackup`
- `backupzomega`
- `zomegabackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 OMEGA, down  to 500 OMEGA.
...
- The `-reservebalance` configuration/startup option has been removed ([#1373](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/pull/1373)).


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/blob/master/depends/README.md) provides 5.9.7)
...
- "moneysupply" and "zomegaSupply" attributes in `getblock` output are removed.
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in omegacoin.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in omegacoin.conf
     "haveconnections": true|false,      (boolean) whether network connections are present
     "mnsync": true|false,               (boolean) whether masternode data is synced
     "walletunlocked": true|false,       (boolean) whether the wallet is unlocked
     "stakeablecoins": n,                (numeric) number of stakeable UTXOs
     "stakingbalance": d,                (numeric) OMEGA value of the stakeable coins (minus reserve balance, if any)
...
### GUI
 ...
 - #1306 `65ba128634` [GUI] Hide privacy widget when the wallet has no zOmega balance (furszy)
 - #1316 `bcb04a44a1` [Trivial] fix typo "recomended" in sendcustomfee dialog (random-zebra)
 - #1323 `e2c07184f9` [Qt] Hide zOmega balances when they are zero (Fuzzbawls)
...
### Wallet
 ...
 - #1382 `df2db0d5c6` [Wallet] Don't initialize zomegawallet on first run (Fuzzbawls)
...
### Core Systems
 ...
 - #1308 `ca912fc823` [zOmega] Public coin, a super for-each removed (furszy)
...
### Cleanup
 ...
 - #1290 `2ceeb2cca0` [zOmega][Cleanup] Zerocoin Cleanup 1: remove Accumulators values (random-zebra)
 - #1291 `16d7dac5f7` [zOmega][Cleanup] Zerocoin Cleanup 2: remove CZOmegaStake class (random-zebra)
 - #1293 `28e0048b3e` [zOmega][Cleanup] Zerocoin Cleanup 3: remove old ZK proofs (random-zebra)
 - #1314 `185194bd7e` [zOmega][Cleanup] Zerocoin Cleanup 4: further wallet cleaning (random-zebra)
 - #1322 `5bd387e18d` [zOmega][Cleanup] Zerocoin Cleanup 5: further main.cpp cleaning (random-zebra)
 - #1326 `6bbd575860` [zOmega][Cleanup] Zerocoin Cleanup 6: Remove zerocoin mint checks (random-zebra)
