OMEGACOIN Core version 3.0.4 is now available from:
...
OMEGACOIN Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zOmega protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
---------------------
The stake miner code now filters out zOmega double spends that were on rare occasions being slipped into blocks (and being rejected by peers when broadcast to the network).
...
More Extensive Display of zOmega Confirmation/Maturity Status
---------------------
Within the Privacy Dialog of the QT wallet, the _Zerocoin Stats_ section now displays both the confirmation and maturity status of minted zOmega's. Previously this was only able to be viewed within the zOmega Control dialog. The main Overview tab of the QT wallet now splits zOmega balance into subcategories: _Unconfirmed_, _Immature_, and _Mature_.
...
- #294 `27c0943` Add additional checks for txid for zomega spend. (presstab)
...
- #314 `f1c830a` Fix issue causing crash when omegacoind --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] OMEGACOIN not responding on user quitting app (Aaron Langford)
