OMEGACOIN Core version *v5.0.0* is now available from:  <https://github.com/Cobra-Technologies-GmbH/OmegaCoin/releases>
...
OMEGACOIN Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Overview
Users will be able to protect their financial information by sending and receiving OMEGAs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://coin.omega.eco .
...
* Top bar showing shield OMEGA balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield OMEGA balance.
...
See the release notes from [v4.2.0](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed OMEGA supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in OMEGA
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in OMEGA
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zOmegasupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield OMEGA)
  The `moneysupply` JSON return entry is the total OMEGA supply (the sum of all transparent and shield OMEGA)
...
| `getaccountaddress` | Returned the current OMEGACOIN address associated with an internal account | Account system removal |
| `setaccount` | Associate a OMEGACOIN address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given OMEGACOIN address | Account system removal |
| `getaddressesbyaccount` | Returned a list of OMEGACOIN addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a OMEGACOIN address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a OMEGACOIN address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zOmega balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zOmega mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zOmega mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zOmega amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zOmega to a OMEGA address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zOmega mints to a OMEGA address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zOmegas to see if any mints were added to the chain | Zerocoin deprecation |
| `setzomegaseed` | Set the wallet's deterministic zOmega seed to a specific value | Zerocoin deprecation |
| `getzomegaseed` | Return the current deterministic zOmega seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zOmega seed | Zerocoin deprecation |
| `dzomegastate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzomega` | Do a search for deterministically generated zOmega that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_omegacoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake omegacoind and unit tests: fixing missing MacOS framework. (furszy)
