...
*Setup instructions for a gitian build of OMEGACOIN Core using a VM or physical system.*
...
to the OMEGACOIN GitHub release page.
...
```bash
gpg --output ${VERSION}-linux/${NAME}/omegacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/omegacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/omegacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/omegacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/omegacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/omegacoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/omegacoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@omegacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/omegacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/omegacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/omegacoin-osx-*-build.assert
```
