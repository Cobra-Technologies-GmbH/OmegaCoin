...
The build script we'll be using is contained in the OMEGACOIN github repository ([contrib/gitian-build.py](https://github.com/Cobra-Technologies-GmbH/OmegaCoin/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the OMEGACOIN repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/OMEGACOIN-Project/OMEGACOIN/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, omegacoin-detached-sigs, and omegacoin GitHub repos.
